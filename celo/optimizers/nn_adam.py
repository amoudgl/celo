...
"""
NN Adam learned optimizer.
https://arxiv.org/abs/2203.11860

A learned LSTM which controls hyper-parameters of Adam.

Adapted from:
https://github.com/google/learned_optimization/blob/2415c13261bfe08569127dd68e38c8ea1f2bf19c/learned_optimization/learned_optimizers/nn_adam.py
"""
...
def _first_second_rolling() -> _InitUpdate:
    ...
    def update_fn(state, grad, beta1, beta2):
        m = jax.tree_util.tree_map(lambda a, b, b1: b1 * a + (1 - b1) * b, state.m, grad, beta1)
        rms = jax.tree_util.tree_map(
            lambda a, b, b2: b2 * a + (1 - b2) * (b * b), state.rms, grad, beta2
        )
...
_scaled_lr = _Invertable(
    forward=lambda x: 0.1 * jnp.log(x), inverse=lambda x: jnp.clip(jnp.exp(10.0 * x), 1e-8, 1e3)
)

_scaled_epsilon = _Invertable(
    forward=lambda x: 0.1 * jnp.log(x), inverse=lambda x: jnp.clip(jnp.exp(10.0 * x), 1e-11, 1e4)
)
...
def _sorted_values(value_dict: Mapping[str, jnp.ndarray]) -> Sequence[jnp.ndarray]:
...
class NNAdam(lopt_base.LearnedOptimizer):
    ...
    def __init__(
        ...
        def _rnn_to_controls(hidden):
            mod = hk.Linear(4, name="rnn_to_controls", w_init=hk.initializers.Constant(0.0))
            return mod(hidden)
    ...
    def opt_fn(self, theta: lopt_base.MetaParams, is_training: bool = True) -> opt_base.Optimizer:
        ...
        class _Opt(opt_base.Optimizer):
            ...
            def init(
                ...
                return NNAdamState(
                    ...
                    per_layer_lr=jax.tree_util.tree_map(lambda x: theta["per_layer_lr"], params),
            ...
            def update(
                ...
                **kwargs
                ...
                b1 = jax.tree_util.tree_map(
                    _scaled_one_minus_log.inverse, opt_state.per_layer_beta1
                )
                b2 = jax.tree_util.tree_map(
                    _scaled_one_minus_log.inverse, opt_state.per_layer_beta2
                )
                epsilon = jax.tree_util.tree_map(
                    _scaled_epsilon.inverse, opt_state.per_layer_epsilon
                )
                ...
                # per tensor).
                next_rolling_features = rolling.update(opt_state.rolling_features, grads, b1, b2)
                ...
                update = jax.tree_util.tree_map(compute_step_for_tensor, m, rms, lr, epsilon)

                summary.tree_step("nn_adam_step", update)

                # apply the step to current parameters
                new_params = jax.tree_util.tree_map(lambda a, b: a - b, opt_state.params, update)
                ...
                deltas = parent.rnn_to_controls.apply(theta["rnn_to_controls_params"], lstm_out)
                treedef = jax.tree_util.tree_structure(opt_state.params)
                deltas = jax.tree_util.tree_unflatten(treedef, list(deltas))

                deltas = jax.tree_util.tree_map(lambda x: x * parent.output_scale, deltas)
