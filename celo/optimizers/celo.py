...
"""
Celo learned optimizer.

A simple learned optimizer containing a learned LSTM scheduler
which controls learned MLP update rule output with a scalar.

Some parts such as accumulators and param features are borrowed from VeLO:
...
class Celo(lopt_base.LearnedOptimizer):
    ...
    def get_frozen_param_keys(self):
        if self.train_phase and self.train_phase == 1:
            return ["lstm_init_state", "rnn_params"]
    ...
    def opt_fn(self, theta, is_training=True) -> opt_base.Optimizer:
        ...
        class _Opt(opt_base.Optimizer):
            ...
            def update(
                ...
                # one per param.
                control_params = [jax.nn.softmax(d) for d in control_params]
