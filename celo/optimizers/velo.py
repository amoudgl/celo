...
"""
VeLO learned optimizer.
https://arxiv.org/abs/2211.09760

A hyper-network based learned optimizer with a per-tensor LSTM
which predicts weights of the MLP update rule through a hyper network.

Adapted from:
https://github.com/google/learned_optimization/blob/2415c13261bfe08569127dd68e38c8ea1f2bf19c/learned_optimization/research/general_lopt/hyper_v2.py

Clone of hyper_v2.py from learned optimization repo above with a minor change:
We expose hard-coded 100.0 factor in velo used for MLP param interpolation as
an argument. In our experiments, we observed instability in PES meta-training
on simple tasks with default 100.0 value, setting it to 1.0 works better.
"""
...
class HyperV2(lopt_base.LearnedOptimizer):
    ...
    def __init__(
        ...
        self.rnn_to_controls = hk.without_apply_rng(
            hk.transform(
                lambda x: hk.Linear(  # pylint: disable=unnecessary-lambda, g-long-lambda
                    param_inits,
    ...
    def _ff_mod(
        ...
        for wi, w in enumerate([self.ff_hidden_size] * (self.ff_hidden_layers) + [3]):
            ...
            if wi == 0:
                ...
                weights.append(
                    hk.get_parameter(f"w{wi}", shape=(last_size, w), dtype=jnp.float32, init=w_init)
                )
        ...
        summary.summary(f"velo/{summary_prefix}/param_scale", param_scale)
        ...
        if self.clip_param_scale_amount is not None:
            ...
            summary.summary(f"velo/{summary_prefix}/post_param_scale", param_scale)

        avg_step_size = jnp.mean(jnp.abs(direction * mag_param * self.step_mult * extra_step_mult))
        summary.summary(f"velo/{summary_prefix}/no_parammag_mult_avg_step_size", avg_step_size)
        ...
        summary.summary(f"velo/{summary_prefix}/pre_precondition_avg_step_size", avg_step_size)
        ...
        avg_step_size = jnp.mean(jnp.abs(step))
        summary.summary(f"velo/{summary_prefix}/avg_step_size", avg_step_size)
        summary.summary(f"velo/{summary_prefix}/extra_step_mult", extra_step_mult)
    ...
    def lstm_features_for_tensor(
        ...
        inputs["var_m"] = _clip_log_abs(var_m, scale=10.0)

        mean_rms = jnp.mean(rms, axis=leading_axis, keepdims=True)
        var_rms = jnp.mean(jnp.square(rms - mean_m), axis=leading_axis)
        inputs["mean_rms"] = _clip_log_abs(jnp.reshape(mean_rms, [mean_rms.shape[-1]]), scale=10.0)
        inputs["var_rms"] = _clip_log_abs(var_rms, scale=10.0)
        ...
        #             summary.summary(f"per_tensor_feat/{k}__{vi}", vv, aggregation="sample")
        #     else:
        ...
        class _Opt(opt_base.Optimizer):
            ...
            def update(
                ...
                rnn_inputs = jax.tree_util.tree_map(
                    ff, opt_state.params, grads, m, rms, summary_prefix
                )
                ...
                def apply_one(
                    ...
                    def interpolate_theta(ff_p):
                        ...
                        out = jnp.mean(ff_p * c, axis=0)
                        out = out * parent.ff_mult
                        return out

                    ff_param = jax.tree_util.tree_map(interpolate_theta, theta["ff_mod_stack"])
