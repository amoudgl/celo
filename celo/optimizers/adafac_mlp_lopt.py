...
Adapted from:
https://github.com/google/learned_optimization/blob/2415c13261bfe08569127dd68e38c8ea1f2bf19c/learned_optimization/learned_optimizers/adafac_mlp_lopt.py
"""
...
def decay_to_param(x):
    return jnp.log(1 - x) / 10.0


def param_to_decay(x):
    return 1 - jnp.exp(x * 10.0)
...
class AdafacMLPLOpt(lopt_base.LearnedOptimizer):
    ...
    def _mod(self, global_feat, p, g, m, rms, fac_g, fac_vec_col, fac_vec_row, fac_vec_v):
        ...
        if factored_dims is not None:
            ...
            fac_mom_mult = (
                m * jnp.expand_dims(row_factor, axis=d0) * jnp.expand_dims(col_factor, axis=d1)
            )
        ...
        for wi, w in enumerate([self._hidden_size] * self._hidden_layers + [2]):
            ...
            make_full_weights = self._concat_weights or (not self._make_separate_weights)
            if make_full_weights:
                weights.append(
                    hk.get_parameter(f"w{wi}", shape=(last_size, w), dtype=jnp.float32, init=w_init)
                )
                biases.append(
                    hk.get_parameter(f"b{wi}", shape=(w,), dtype=jnp.float32, init=jnp.zeros)
                )
            else:
        ...
        if self._concat_weights:
            ...
            stacked = jnp.reshape(
                training_step_feature, [1] * len(axis) + list(training_step_feature.shape[-1:])
            )
            ...
            inp = [
                x * lax.rsqrt(1e-5 + jnp.mean(jnp.square(x), keepdims=True)) for x in flat_features
            ]
            for wi, (w, b) in enumerate(zip(weights, biases)):
                grids = []
                ...
                for oi in range(len(w[0])):
                    ...
                    if wi == 0:
                        training_step_feature = global_feat["training_step_feature"]
                        for i, vi in enumerate(range(vi + 1, vi + 1 + len(training_step_feature))):
                            if type(w) == list:  # pylint: disable=unidiomatic-typecheck
        ...
        summary.summary("adafac_mlp_lopt/avg_step_size_hist", avg_step_size, aggregation="collect")
        summary.summary("adafac_mlp_lopt/direction/mean_abs", jnp.mean(jnp.abs(direction)))
        summary.summary("adafac_mlp_lopt/magnitude/mean_abs", jnp.mean(jnp.abs(magnitude)))
    ...
    def init(self, key: PRNGKey) -> lopt_base.MetaParams:
        ...
        mod_theta = self._mod_init(
            key, global_features, p, g, m, rms, fac_g, fac_vec_col, fac_vec_row, fac_vec_v
        )
        ...
        class _Opt(opt_base.Optimizer):
            ...
            def _get_rolling(self):
                mom_decay = param_to_decay(
                    decay_to_param(
                        jnp.asarray(parent._initial_momentum_decays)
                ...
                rms_decay = param_to_decay(
                    decay_to_param(
                        jnp.asarray(parent._initial_rms_decays)
                    )  # pylint: disable=protected-access
                ...
                adafactor_decay = param_to_decay(
                    decay_to_param(
                        jnp.asarray(parent._initial_adafactor_decays)
                    )  # pylint: disable=protected-access
