...
"""
RNN MLP learned optimizer.
https://arxiv.org/abs/2009.11243

A hierarchical learned optimizer containing a per-tensor LSTM which
communicates with MLP update rule through conditioning embeddings.

Adapted from:
https://github.com/google/learned_optimization/blob/2415c13261bfe08569127dd68e38c8ea1f2bf19c/learned_optimization/learned_optimizers/rnn_mlp_lopt.py
"""
...
class RNNMLPLOpt(lopt_base.LearnedOptimizer):
    ...
    def __init__(
        ...
        self.rnn_to_mlp_network = hk.without_apply_rng(
            hk.transform(lambda x: hk.Linear(lstm_to_ff, name="rnn_to_ff")(x))
        )  # pylint: disable=unnecessary-lambda
        self.rnn_to_rnn_network = hk.without_apply_rng(
            hk.transform(lambda x: hk.Linear(from_lstm_size, name="rnn_to_in")(x))
        )  # pylint: disable=unnecessary-lambda

        self.lstm_fn = lambda: hk.LSTM(lstm_hidden_size, name="rnn")
        self.rnn_network = hk.without_apply_rng(
            hk.transform(lambda x, state: self.lstm_fn()(x, state))
        )  # pylint: disable=unnecessary-lambda
    ...
    def init(self, key) -> lopt_base.MetaParams:
        ...
        lstm_inital_state = hk.transform(lambda: self.lstm_fn().initial_state(1))[
            1
        ](  # pylint: disable=unnecessary-lambda
            None, key3
        )
        ...
        return {
            "initial_from_lstm": var_init(None, key1, [self.from_lstm_size], dtype=jnp.float32),
            "initial_from_mlp": var_init(None, key2, [self.from_mlp_size], dtype=jnp.float32),
            "lstm_init_state": lstm_inital_state,
            "rnn_params": self.rnn_network.init(
                key3, jnp.zeros([1, rnn_inp_size]), lstm_inital_state
            ),
            "rnn_to_ff_params": self.rnn_to_mlp_network.init(
                key4, jnp.zeros([0, self.lstm_hidden_size])
            ),
            "rnn_to_in_params": self.rnn_to_rnn_network.init(
                key5, jnp.zeros([0, self.lstm_hidden_size])
            ),
            "ffmod_params": self.per_param_mlp_network.init(key6, jnp.zeros([0, mlp_inp_size])),
        }
        ...
        class _Opt(opt_base.Optimizer):
            ...
            def update_is_valid(self, opt_state, loss) -> RNNMLPLOptState:
                ...
                next_opt_state = opt_state.replace(
                    iteration=opt_state.iteration + 1, valid_loss_accum=next_valid_loss_accum
                )
                return tree_utils.match_type(next_opt_state, opt_state)

            def update_is_training(self, opt_state, grads, loss, model_state) -> RNNMLPLOptState:
                ...
                def to_map_per_tensor(ms, rms, g, v, from_mlp):
                    return self.lstm_features_for_tensor(
                        ms, rms, g, v, from_mlp, from_lstm, train_loss_feat, valid_loss_feat
                    )
                ...
                # RNN only once.
                rnn_inputs = jnp.stack([jnp.concatenate(v, axis=0) for v in stacked_inp_tree])
                ...
                new_from_lstm = jnp.mean(
                    parent.rnn_to_rnn_network.apply(theta["rnn_to_in_params"], lstm_out), axis=0
                )

                # Compute values passed from the lstm into the FF network.
                ff_inputs = parent.rnn_to_mlp_network.apply(theta["rnn_to_ff_params"], lstm_out)
                ...
                from_mlp = []
                for o, v in zip(
                    jax.tree_util.tree_leaves(outputs), jax.tree_util.tree_leaves(opt_state.params)
                ):
                    direction = o[:, 0:1]
                    magnitude = o[:, 1:2]
                    step = (
