...
"""
Updates gradient learner in learned optimization library to support
freezing of some learned optimizer params specified by 'frozen_keys'.

Adapted from:
https://github.com/google/learned_optimization/blob/2892faec4d5b24a419b6892056af8ee6f7920309/learned_optimization/outer_trainers/gradient_learner.py
"""
...
from learned_optimization import checkpoints, profile, summary, tree_utils
...
class GradientLearner:
    ...
    def __init__(
        ...
        frozen_keys: Optional[Sequence[str]] = None,
        init_theta: Optional[chex.ArrayTree] = None,
    ...
    def get_meta_model_state(self, state: GradientLearnerState) -> ThetaModelState:
    ...
    def init(self, key: PRNGKey) -> GradientLearnerState:
        ...
        model_state = None
        if self._init_theta:
            theta_init = self._init_theta
            logging.info("Initialized params from the checkpoint.")
        else:
            theta_init = self._meta_init.init(key)

        theta_opt_state = self._theta_opt.init(theta_init, model_state, num_steps=self._num_steps)
        ...
        if self._init_outer_state_from_path:
            ...
            real_checkpoint = checkpoints.load_state(
                self._init_outer_state_from_path, fake_checkpoint
            )
        ...
        if self._frozen_keys:
            ...
            def get_mask_val(k, v):
                val = 1.0
                for ki in self._frozen_keys:
                    if ki in k:
                        val = 0.0
                        break
                return val

            frozen_mask = tree_utils.map_named(lambda k, v: get_mask_val(k, v), theta_init)
    ...
    def update(
        ...
        with profile.Profile("stack_state"):
            model_state_stack = tree_utils.tree_zip_onp([t.theta_model_state for t in grads_list])
        ...
        if self._frozen_keys:
            grads = jax.tree_util.tree_map(lambda m, g: m * g, self._frozen_mask, grads)
        ...
        return (
            GradientLearnerState(theta_opt_state),
            metrics,
...
def _jit_nan_to_num(vals, replace):
    return jax.tree_util.tree_map(
        functools.partial(jnp.nan_to_num, nan=replace, posinf=replace, neginf=replace), vals
    )
...
def _tree_zeros_on_device(shapes, device):
    ...
    return jax.tree_util.tree_unflatten(treedef, _tree_zeros_on_device_inner(tuple(leaves), device))


@functools.partial(jax.jit, static_argnums=(0, 1))
def _tree_zeros_on_device_inner(shapes, device):
    zero_val = lambda x: jax.device_put(jnp.asarray(0, dtype=x.dtype), device)
    return jax.tree_util.tree_map(lambda x: jnp.full(x.shape, zero_val(x)), shapes)
...
def gradient_worker_compute(
    ...
    theta_shape = jax.tree_util.tree_map(lambda x: core.ShapedArray(x.shape, x.dtype), theta)
    ...
    for si, (estimator, unroll_state) in enumerate(zip(gradient_estimators, unroll_states)):
        with profile.Profile(f"estimator{si}"):
            ...
            if estimator_out.unroll_info:
                ...
                iteration = (
                    estimator_out.unroll_info.iteration[idx]
                    if estimator_out.unroll_info.iteration is not None
                    else None
                )
                event_info.append(
                    {
                ...
                logging.warn("No out specified by learner. " "Not logging any events data.")
...
class SingleMachineGradientLearner:
    ...
    def __init__(
        ...
        self.gradient_learner = GradientLearner(
            meta_init, theta_opt, frozen_keys=frozen_keys, num_steps=num_steps
        )
    ...
    def init(self, key: PRNGKey) -> SingleMachineState:
        ...
        return SingleMachineState(
            gradient_learner_state=theta_state, gradient_estimator_states=unroll_states
        )

    def update(
        self, state, key: PRNGKey, with_metrics: Optional[bool] = False
        ...
        """
        key1, key2 = jax.random.split(key)
        worker_weights = self.gradient_learner.get_state_for_worker(state.gradient_learner_state)
        ...
        next_theta_state, metrics = self.gradient_learner.update(
            ...
            [worker_compute_out.to_put],
            key=key2,
            with_metrics=with_metrics,
        )

        metrics = summary.aggregate_metric_list([worker_compute_out.metrics, metrics])
        ...
        return (
            ...
            worker_compute_out.to_put.mean_loss,
            metrics,
        )
