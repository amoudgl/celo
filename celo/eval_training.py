...
"""
Adapted from:
https://github.com/google/learned_optimization/blob/2892faec4d5b24a419b6892056af8ee6f7920309/learned_optimization/eval_training.py

Fixes a minor bug in `eval_training.py` in learned_optimization:
If `metrics_every` is set and is not equal to 1, single_task_training_curves
gives an error at the end of evaluation while zipping metrics dictionary.
The fix is to append to metrics list at `metrics_every` interval not 1
as in the original implementation (line 227).
"""
...
from jax import lax
from learned_optimization import jax_utils, profile, summary, training, tree_utils
...
def _next_state(
    ...
    def fn(opt_state, key, data):
        ...
        (l, state), grad = jax.value_and_grad(task.loss_with_state, has_aux=True)(p, s, key1, data)
                ...
                if summary_writer:
                    for k, v in m.items():
                        ...
                        if agg in ["mean", "sample"]:
                            ...
                            logging.warning(
                                f"Not supported aggregation type {agg}."  # pylint: disable=logging-fstring-interpolation
...
class _CachedTrainFun:
    init: Callable[[lopt_base.MetaParams, PRNGKey, int], Tuple[OptState, TaskParam]]
    init_with_task_params: Callable[[lopt_base.MetaParams, PRNGKey, int, TaskParam], OptState]
    unroll_n_steps: Callable[
        [lopt_base.MetaParams, OptState, TaskParam, Tuple[Data, PRNGKey]],
        Tuple[OptState, jnp.ndarray, jnp.ndarray],
    ]
    eval_loss: Callable[
        [lopt_base.MetaParams, TaskParam, OptState, Tuple[Any, PRNGKey]], jnp.ndarray
    ]
...
def _cached_vectorize_train_fns(
    ...
    def vec_single_task(theta, key, num_steps):
        ...
        def fn(key):
            ...
            inner_param, inner_state = task_family.task_fn(task_param).init_with_state(key2)
            opt_state = opt.init(
    ...
    def vec_single_task_with_task_params(theta, key, num_steps, task_params):
        ...
        def fn(key, task_param):
            ...
            inner_param, inner_state = task_family.task_fn(task_param).init_with_state(key1)
            opt_state = opt.init(
    ...
    def one_step(opt, task_param, opt_state, data_key):
        ...
        next_opt_state, l, key, _ = _next_state(task, opt, opt_state, data, key, with_metrics=False)
    ...
    def vec_unroll_n_steps(theta, opt_states, task_params, datas_key):
        ...
        def fn(opt_states, task_params, data_key):
            ...
            opt_states, losses = lax.scan(p_one_step, opt_states, data_key, length=steps_per_jit)
            norm_losses = jax.vmap(task_family.task_fn(task_params).normalizer)(losses)
    ...
    def eval_loss(theta, task_params, opt_state, data_key):
        ...
        def fn(opt_state, task_param, data_key):
            ...
            return jnp.mean(loss), jnp.mean(norm_loss), jax.tree_util.tree_map(jnp.mean, aux)
...
def multi_task_training_curves(
    ...
    if n_tasks % n_devices != 0:
        raise ValueError(
            "Must specify n_tasks to be a multiple of n_devices."
    ...
    logging.info(
        f"Running _cached_vectorize_train_fns with: "  # pylint: disable=logging-fstring-interpolation
    ...
    if task_params is None:
        ...
        task_params = jax.tree_util.tree_map(
            lambda x: jnp.reshape(x, (n_devices, n_tasks_per_device) + x.shape[1:]), task_params
        )
        opt_states = train_fns.init_with_task_params(theta, keys, steps, task_params)

    if steps % steps_per_jit:
        raise ValueError(
            "Please set steps and steps_per_jit to be multiples of"
        ...
        with profile.Profile("eval"):
            ...
            def losses_for_split(split):
                ...
                for _ in range(n_eval_batches):
                    ...
                    l, norm_l, auxs = train_fns.eval_loss(
                        theta, task_params, opt_states, (eval_datas, keys)
                    )
                    sub_l.append(l)
                ...
                with profile.Profile("eval_agg_blocked"):
                    ...
                    return (
                        onp.mean(sub_l, axis=0),
                        onp.mean(sub_norm_l, axis=0),
    ...
    for i in tqdm.trange(steps // steps_per_jit):
        if (
            (i * steps_per_jit) % eval_every == 0
            and n_eval_batches_vec > 0
            and (not eval_just_at_end)
        ):
    ...
    with profile.Profile("final_eval"):
        if n_eval_batches_vec > 0:
            data_keys, keys = _pmap_vector_random_split(keys, n_eval_batches_vec)
            l = eval_loop(theta, task_params, opt_states, data_keys, last_eval_batches)
            eval_losses.append(l)
    ...
    return {
        "train/xs": onp.tile(onp.expand_dims(onp.arange(steps), 0), [n_tasks, 1]),
        "train/loss": train_losses,
        "eval/xs": onp.tile(onp.expand_dims(onp.asarray(eval_xs), 0), [n_tasks, 1]),
        "eval/last_eval_batches": onp.asarray(last_eval_batches),
        "eval/eval_batches": onp.asarray(n_eval_batches * n_eval_batches_vec),
        **eval_losses,
    }
