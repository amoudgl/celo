"""Submission file for AlgoPerf evaluation."""
...
from celo.utils import WeightDecayWrapper, cached_jit, load_state
...
def init_optimizer_state(
    ...
    # TIP: can also use weight decay at test-time like below
    # opt = WeightDecayWrapper(opt, weight_decay=0.08121616522670176)

    # create optimizer state
    num_steps = workload.step_hint
    print(f"Step hint: {num_steps}")
...
def pmapped_train_step(
    ...
    # lopt update
    optimizer_state = optimizer_state.replace(params=current_param_container)
    new_optimizer_state = opt_update_fn(optimizer_state, grad, loss)
    updated_params = new_optimizer_state.params
